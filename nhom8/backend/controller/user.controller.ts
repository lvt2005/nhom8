...
export const checkBlockStatus = async (req: RequestAccount, res: Response) => {
  try {
    const myId = req.account._id;
    const { userId } = req.body;

    const me = await account_user.findById(myId);
    const targetUser = await account_user.findById(userId);

    if (!me || !targetUser) {
      return res.json({ code: "error", Message: "Không tìm thấy người dùng" });
    }

    const iBlockedThem = me.blockedUsers?.some((id: any) => id.toString() === userId) || false;
    const theyBlockedMe = targetUser.blockedUsers?.some((id: any) => id.toString() === myId.toString()) || false;
