...
export const sendMessage = async (req: any, res: Response) => {
  try {
    ...
    // Kiểm tra block nếu là chat riêng (không phải nhóm)
    if (type !== 'group') {
      const sender = await account_user.findById(sender_id);
      const receiver = await account_user.findById(receiver_id);
      
      // Kiểm tra nếu người gửi đã bị chặn bởi người nhận hoặc ngược lại
      const isBlockedBySender = sender?.blockedUsers?.some((id: any) => id.toString() === receiver_id);
      const isBlockedByReceiver = receiver?.blockedUsers?.some((id: any) => id.toString() === sender_id);
      
      if (isBlockedBySender || isBlockedByReceiver) {
        return res.json({ 
          code: "blocked", 
          Message: "Bạn không thể liên lạc với người này!" 
        });
      }
    }
