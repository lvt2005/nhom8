...
export const CenterData: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 0. Thông báo kết bạn
    socket.on("SERVER_FRIEND_ACCEPTED", (data: { friendId: string, friendName: string }) => {
      setNotifications(prev => [`${data.friendName} đã chấp nhận lời mời kết bạn!`, ...prev]);
      toast.success(`${data.friendName} đã chấp nhận lời mời kết bạn!`);
  ...
  const handleSelectChat = (id: string, type: 'group' | 'friend') => {
  ...
  const handleSendQuickEmoji = async () => {
    ...
    try {
      ...
      formData.append("type", selectedChatType || "friend");
      await fetch(`${process.env.NEXT_PUBLIC_DOMAIN}/chat/send`, { method: "POST", credentials: "include", body: formData });
  ...
  const handleBlockUser = async () => {
    ...
    if (!confirm("Bạn có chắc muốn chặn người dùng này? Họ sẽ không thể nhắn tin cho bạn và bạn sẽ không tìm thấy họ.")) return;
    try {
